...
- [ ] **Main Window:** Products / Work Orders / MRP tabs.
- [ ] **Products Page** (start with the simplest): Listing with a DataGrid, adding product/BOM via form controls. → run and verify when complete.
- [ ] **Work Orders Page:** Multi-line order creation form. → run and verify.
- [ ] **MRP Page** (most complex, leave for last): Hierarchical summary (TreeDataGrid/ListBox) + Excel download button.

---

## Phase 6 — Packaging and Windows Testing (periodic, not just at the end)

- [ ] End of Phase 2: Take another `win-x64` publish, open it on Windows.
- [ ] End of Phase 3: Repeat the same test.
- [ ] Final packaging: `dotnet publish -r win-x64 -c Release /p:PublishSingleFile=true --self-contained true`
...
- [ ] Add an "Import from Excel" button to the Work Orders page.
- [ ] Write `ExcelImportService` using ClosedXML to read rows (Width, Height, Length, Quantity) from the uploaded `.xlsx` file.
- [ ] Automatically generate a Work Order from the read data and run `MrpService` to instantly present the results.
- [ ] Test the import with sample takeoff lists (metraj listesi) to ensure data mapping accuracy.

---

## Verification Checklist

- [ ] Does the C# project compile and open without errors?
- [ ] Are the records in the existing SQLite listed correctly in the UI?
- [ ] Is a newly added product saved correctly to the database?
- [ ] Is the MRP calculation on the same products **exactly identical** to the Python version? (Using `decimal` guarantees this)
- [ ] Is the visual output of the Excel file correct?
- [ ] Does the packaged `.exe` open when double-clicked on a real Windows machine?
...
| Phase 6 — Packaging | ⬜ Not Started |
| Phase 7 — Excel Import | ⬜ Not Started |
